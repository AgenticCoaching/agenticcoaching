...
In Claude, Claude in Chrome MCP is required for this setup skill. If Claude in Chrome is not installed and enabled, stop and ask the user to install or enable it before proceeding. In Codex, perform the same steps with manual browser guidance instead of requiring Claude in Chrome.
...
1. In Claude, use Claude in Chrome MCP to open the existing personal app or create it if it does not exist at `https://www.strava.com/settings/api`. In Codex, guide the user through the same page manually.
2. When creating or repairing the personal app, upload the connector logo from:
   - `/Users/arne/Development/AgenticCoaching/AgenticCoaching/plugins/strava-connector/assets/AgenticCoachingLogo.png`
